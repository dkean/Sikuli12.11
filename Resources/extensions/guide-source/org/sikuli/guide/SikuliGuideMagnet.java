...
public class SikuliGuideMagnet
        implements Transition, GlobalMouseMotionListener {

  SikuliGuide guide;
  GlobalMouseMotionTracker mouseTracker;
  private SikuliGuideClickable lastClickedClickable;

  public SikuliGuideMagnet(SikuliGuide guide) {
    this.guide = guide;
    ...
    guide.addComponent(new SikuliGuideClickable(), 0);
...
//      }
  ...
  void attractTarget(SikuliGuideAnchor a, Point targetLocation) {
    ...
    try {
      ...
      SikuliGuideClickable clickable = new SikuliGuideClickable();
...
//      Debug.log("[SikuliGuideMagnet] moved to " + x + "," + y);

    Point p = new Point(x, y);
    for (Link link : links) {

      if (link.image.getActualBounds().contains(p)) {
        //Debug.info("[SikuliGuideMagnet] mouseover on a target");
