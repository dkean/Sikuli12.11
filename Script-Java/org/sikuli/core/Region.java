...
public class Region {
  ...
  public Region copyTo(int scrID) {
    ...
    return Screen.getScreen(scrID).newRegion(new Rectangle(x - old.x, y - old.y, w, h));
  }
  ...
  public Region copyTo(Screen screen) {
    Location old = (Location) getScreen().getBounds().getLocation();
    return screen.newRegion(new Rectangle(x - old.x, y - old.y, w, h));
  }

  /**
   * used in EventManager.callChangeObserver, Finder.next
   * to adjust region relative coordinates of matches to screen coordinates
   * @param m
   * @return
   */
