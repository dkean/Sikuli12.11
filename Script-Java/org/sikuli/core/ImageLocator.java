...
public class ImageLocator {
	 ...
	 * path's from -DSIKULI_IMAGE_PATH=... <br />list[n+1:] path's added later <br
	 * />each path is contained only once <br />each contained file path existed
	...
	private static String searchFile(String filename) {
		...
		for (Iterator<String> it = pathList.iterator(); it.hasNext();) {
			...
			if (f.exists()) {
				Debug.log(2, "ImageLocator: found " + filename + " in " + path);
	...
	/**
	 * find the file in the following order: <br />1. absolute path or url x) <br
	 * />2. bundle path <br />3. ENV[SIKULI_IMAGE_PATH] <br />4. Java
